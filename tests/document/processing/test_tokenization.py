...
def test_text_based_document_to_token_based(document_dict, tokenizer):
    for doc_id in ["train_doc1", "train_doc2", "train_doc3"]:
        ...
        tokenized_doc = text_based_document_to_token_based(
            ...
            token_offset_mapping=None if doc.id == "train_doc2" else tokenized_text.offset_mapping,
            # to increase test coverage
            char_to_token=None if doc.id == "train_doc1" else tokenized_text.char_to_token,
        )
...
def test_text_based_document_to_token_based_missing_args(document_dict, tokenizer):
    with pytest.raises(ValueError) as excinfo:
        doc = document_dict["train_doc1"]
...
def test_text_based_document_to_token_based_unaligned_span_strict(document_dict, tokenizer):
    doc = document_dict["train_doc1"].copy()
...
def test_text_based_document_to_token_based_unaligned_span_not_strict(document_dict, tokenizer):
    doc = document_dict["train_doc1"].copy()
    doc.entities.append(LabeledSpan(start=0, end=2, label="unaligned"))
    assert str(doc.entities[-1]) == "A "
    tokenized_text = tokenizer(doc.text, return_offsets_mapping=True)
